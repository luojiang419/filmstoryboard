...
class NanoBananaReplicationPromptCompiler {
  ...
  String compile(NanoBananaReplicationPromptInput input) {
    ...
    return <String>[
      ...
      ...authorityLines,
      sceneRule,
      '【编辑方法】以图片1为编辑底图，按下方正文逐项执行“保留主体、替换主体、移除主体、保留白名单元素”；只改动被明确指定的实体。保留人物仅锁定其身份，服装按同编号服装槽独立执行替换或保留；移除后按相邻透视、纹理、材质、遮挡与光影自然补全；新增或替换实体必须匹配图片1的尺度、位置、姿态、接触、镜头；景深和光照在有场景资产时取场景资产，否则取图片1。',
      '同一逻辑资产的多张视图只是互补证据：主视图锁定整体身份、轮廓和比例，细节视图只补充局部结构与材质。不得生成混合身份、混合产品、额外副本，模特与服装素材的背景、版式、机位、姿势、光照或调色不得带入成图；场景素材明确提供最终背景、环境光与景深，必须使用。',
      markWhitelist,
      '【确定性复刻正文】',
      automaticPrompt,
