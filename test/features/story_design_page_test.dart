...
void main() {
  testWidgets('设计分镜图左栏可拖拽且矮窗口仍显示生成按钮', (tester) async {
    ...
    final referenceDropTargetFinder = find.byKey(
      const ValueKey('story-design-reference-drop-target'),
    );
    ...
    final referenceImage = File('assets/branding/app_icon_512.png').absolute;
    referenceDropTarget = tester.widget<DropTarget>(referenceDropTargetFinder);
    referenceDropTarget.onDragDone!(
      DropDoneDetails(
        files: [DropItemFile(referenceImage.path)],
        localPosition: Offset.zero,
        globalPosition: Offset.zero,
      ),
    );
    await tester.pump();
    expect(storyDesignController.value.referenceImagePaths, [
      referenceImage.path,
    ]);
    expect(find.text('已添加 1 张参考图'), findsOneWidget);
    expect(
      find.byKey(const ValueKey('story-design-reference-0')),
      findsOneWidget,
    );
  ...
  testWidgets('设计分镜结果右键可添加为资产库资产', (tester) async {
    ...
    await tester.runAsync(() async {
      ...
      resultFile = File('assets/branding/app_icon_512.png').absolute;
      storyDesignController = StoryDesignController(
        directories: directories,
        settingsController: settingsController,
        gridCutController: gridCutController,
    ...
    await tester.pumpAndSettle();
    final assetName = find.byKey(const ValueKey('story-design-add-asset-name'));
    expect(assetName, findsOneWidget);
    await tester.enterText(assetName, '设计资产');
