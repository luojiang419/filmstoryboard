...
void main() {
  ...
  testWidgets('故事板恢复损坏的编组环时仍可切页渲染', (tester) async {
    late final Directory root;
    late final AppDatabase database;
    await tester.runAsync(() async {
      root = await Directory.systemTemp.createTemp('storyboard_cycle_');
      database = await AppDatabase.open(
    ...
    expect(
      controller.value.resourceGroups.every(
        (group) => group.parentGroupId != group.id,
      ),
      isTrue,
    );
  });
